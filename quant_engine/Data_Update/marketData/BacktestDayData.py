...
class BacktestDayData:
    def __init__(self):
        ...
        self.measure = 'marketData'

    def process_data(self, start_input, end_input, n_jobs):
        # 获取股票、指数、期货高开低收量价状态
        ohlc = self.rdf.get_ohlc(start_input=start_input, end_input=end_input)
        ...
        merged_data = pd.concat([merged_data, IH_weight['IH_weight'], IF_weight['IF_weight'], IC_weight['IC_weight']],
                                axis=1)
        ...
        for idx, row in st.iterrows():
            if pd.isnull(row['exit_date']):
                merged_data.loc[(row['code'], row['entry_date']):(row['code'], dtparser.parse(str(end_input))),
                'isST'] = True
            else:
        ...
        for idx, row in citics_lv1.iterrows():
            if pd.isnull(row['exit_date']):
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'citics_lv1_code'] = row['index_lv1_code']
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'citics_lv1_name'] = row['industry_lv1_name']
            else:
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
                'citics_lv1_code'] = row['index_lv1_code']
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
                'citics_lv1_name'] = row['industry_lv1_name']
        ...
        for idx, row in citics_lv2.iterrows():
            if pd.isnull(row['exit_date']):
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'citics_lv2_code'] = row['index_lv2_code']
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'citics_lv2_name'] = row['industry_lv2_name']
            else:
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
                'citics_lv2_code'] = row['index_lv2_code']
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
        ...
        for idx, row in citics_lv3.iterrows():
            if pd.isnull(row['exit_date']):
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'citics_lv3_code'] = row['index_lv3_code']
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'citics_lv3_name'] = row['industry_lv3_name']
            else:
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
                'citics_lv3_code'] = row['index_lv3_code']
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
        ...
        for idx, row in sw_lv1.iterrows():
            if pd.isnull(row['exit_date']):
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'sw_lv1_code'] = row['index_lv1_code']
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'sw_lv1_name'] = row['industry_lv1_name']
            else:
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
                'sw_lv1_code'] = row['index_lv1_code']
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
        ...
        for idx, row in sw_lv2.iterrows():
            if pd.isnull(row['exit_date']):
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'sw_lv2_code'] = row['index_lv2_code']
                merged_data.loc[
                (row['stock_code'], row['entry_date']):(row['stock_code'], dtparser.parse(str(end_input))),
                'sw_lv2_name'] = row['industry_lv2_name']
            else:
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
                'sw_lv2_code'] = row['index_lv2_code']
                merged_data.loc[(row['stock_code'], row['entry_date']):(row['stock_code'], row['exit_date']),
        ...
        merged_data = merged_data.groupby(['date', 'code']).last()
        merged_data = merged_data.reset_index().set_index('date')
