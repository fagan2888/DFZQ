...
class IndustryNeutralEngine:
    ...
    def run(self,stk_indu_weight,start,end,benchmark,adj_interval,
        ...
        nxt_day_benchmark_weight = daily_data.loc[:, ['date', 'code', benchmark + '_weight']].copy()
        nxt_day_benchmark_weight.rename(
            columns={'date': 'next_1_day', benchmark + '_weight': 'next_bm_weight'},inplace=True)
        daily_data = pd.merge(daily_data,nxt_day_benchmark_weight,on=['next_1_day','code'])
        day_industry_weight = pd.DataFrame(daily_data.groupby(['date','industry'])['next_bm_weight'].sum())
        day_industry_weight.reset_index(inplace=True)
        day_industry_weight.rename(columns={'next_bm_weight':'industry_weight'},inplace=True)
        # 现将股票行业内权重与行业权重合并
        stk_indu_weight.index.names = ['date']
        stk_indu_weight.reset_index(inplace=True)
        stk_indu_weight = pd.merge(stk_indu_weight,day_industry_weight,on=['date','industry'])
        # 把行情数据中的industry删掉，以免跟权重中的industry字段冲突
        daily_data.drop('industry',axis=1,inplace=True)
        # 合并stk indu weight
        daily_data = pd.merge(daily_data,stk_indu_weight,on=['date','code'],how='outer')
        ...
        for trade_day in trade_days:
            ...
            portfolio_value_dict[trade_day] = \
                {'Balance':self.stk_portfolio.balance, 'StockValue':total_value-self.stk_portfolio.balance,
                 'TotalValue':total_value, 'DelistAmount':delist_amount, 'SuspendedStks':suspend_stks_in_pos.shape[0]}
