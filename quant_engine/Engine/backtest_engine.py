...
class BacktestEngine:
    ...
    def run(self,stk_weight,start,end,cash_reserve_rate=0.05,price_field='vwap'):
        ...
        daily_data = daily_data.loc[daily_data['code'].isin(stk_weight['code']),
            ...
            'cash_dvd_ratio','conversed_ratio','rightissue_price','rightissue_ratio','vwap']]
        ...
        for trade_day in trade_days:
            ...
            for idx,row in one_day_data.iterrows():
                if row['status'] == '停牌':
                    pass
                else:
...
if __name__ == '__main__':
    '''
    influx = influxdbData()
    d = pd.concat(influx.getDataMultiprocess('DailyData_backtest','marketData','20170101','20190831'))
    d = d.loc[pd.notnull(d['IH_weight']) & (d['status']=='交易'),['code','IH_weight','vwap']]
    d.columns = ['code','weight','vwap']
    d.to_pickle('20170101_20190831_weight.pkl')
    d = d.loc[:,['code','weight']]
    '''
    d = pd.read_pickle('20170101_20190831_weight.pkl')
    d = d.loc[:,['code','weight']]
    QE = BacktestEngine(stock_capital=100000000)
    portfolio_value_dict = QE.run(d,20170101,20190831,price_field='vwap',cash_reserve_rate=0)
