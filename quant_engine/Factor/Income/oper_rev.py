...
class tot_oper_rev(FactorBase):
    ...
    def cal_factors(self,start,end):
        ...
        query = "select ANN_DT, S_INFO_WINDCODE, REPORT_PERIOD, OPER_REV, STATEMENT_TYPE " \
        ...
        income = pd.DataFrame(self.rdf.curs.fetchall(),
                              columns=['date', 'code', 'report_period', 'oper_rev', 'type'])
        ...
        oper_rev = income.dropna(subset=['oper_rev']).groupby(['code', 'date', 'report_period'])['oper_rev'].last()
        oper_rev = pd.DataFrame(oper_rev).reset_index()
        return oper_rev
...
if __name__ == '__main__':
    ...
    print('data got')
    h5 = pd.HDFStore(global_constant.ROOT_DIR+'Data_Resource/Income/oper_rev.h5', 'w')
    h5['data'] = r
    h5.close()
    print('data saved')