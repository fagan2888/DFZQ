...
class TotAssetsGrowth(FactorBase):
    def __init__(self):
        super().__init__()
    ...
    def cal_factors(self, start, end, n_jobs):
        ...
        fail_list = []
        # 计算 tot_assets 的 growth
        tot_assets = self.influx.getDataMultiprocess('FinancialReport_Gus', 'tot_assets', start, end)
        codes = tot_assets['code'].unique()
        split_codes = np.array_split(codes, n_jobs)
        with parallel_backend('multiprocessing', n_jobs=n_jobs):
            res = Parallel()(delayed(TotAssetsGrowth.JOB_factor)
                             (codes, tot_assets, 'tot_assets', self.db, 'tot_assets_growth')
                             for codes in split_codes)
        print('tot_assets_growth finish')
        print('-' * 30)
        for r in res:
            fail_list.extend(r)
        return fail_list
