...
class Huatai_MF_2(FactorBase):
    ...
    def job_factors(self, df, code, start, end):
        ...
        df[['mv', 'profit_ddt_ttm', 'FCFF', 'EBITDA', 'total_cash', 'total_liab']] = \
            df[['mv','profit_ddt_ttm','FCFF','EBITDA','total_cash','total_liab']].fillna(method='ffill')
        ...
        profit_ttm = df.loc[:,['date','profit_ttm']].copy()
        last_Y_profit_ttm = profit_ttm.copy()
        last_Y_profit_ttm['date'] = last_Y_profit_ttm['date'].apply(lambda x:x+relativedelta(years=1))
        last_Y_profit_ttm.columns = ['date','last_Y_profit_ttm']
        profit_ttm = pd.merge(profit_ttm,last_Y_profit_ttm,left_on='date',right_on='date',how='left')
        ...
        profit_ttm = profit_ttm.loc[:,['date','profit_TTM_YOY']]
        df = pd.merge(df,profit_ttm,left_on='date',right_on='date',how='left')
        df['PEG'] = df['mv']/df['profit_ttm']/df['profit_TTM_YOY']
        df.set_index('date',inplace=True)
        df = df.loc[pd.notnull(df['EP_TTM'])|pd.notnull(df['EPcut_TTM'])|pd.notnull(df['BP'])|pd.notnull(df['SP'])
                    |pd.notnull(df['NCFP'])|pd.notnull(df['OCFP'])|pd.notnull(df['FCFP'])|pd.notnull(df['PEG'])
                    |pd.notnull(df['profit_TTM_YOY']),:]
        df = df.loc[str(start):str(end), ['code', 'EP_TTM', 'EPcut_TTM', 'BP', 'SP', 'NCFP',
    ...
    def cal_factor(self,start,end):
        ...
        FinIndi = FinIndi.sort_values(by=['code','date','report_period'])
        ...
        FinIndi = FinIndi.drop(['report_period'],axis=1)
        ...
        # 起始时间往前推1年
        query = "select s_info_windcode, ANN_DT, REPORT_PERIOD, CASH_CASH_EQU_END_PERIOD " \
        ...
        self.rdf.curs.execute(query)
        CashFlow = pd.DataFrame(self.rdf.curs.fetchall(),columns=['code','date','report_period','total_cash'])
        ...
        # 起始时间往前推1年
        query = "select s_info_windcode, ANN_DT, REPORT_PERIOD, TOT_LIAB " \
            ...
            .format((dtparser.parse(str(start)) - relativedelta(years=1)).strftime('%Y%m%d'), str(end))
        self.rdf.curs.execute(query)
        BalanceSheet = pd.DataFrame(self.rdf.curs.fetchall(), columns=['code', 'date', 'report_period', 'total_liab'])
        BalanceSheet = BalanceSheet.sort_values(by=['code', 'date', 'report_period'])
        BalanceSheet['date'] = pd.to_datetime(BalanceSheet['date'])
        BalanceSheet = BalanceSheet.groupby(['date', 'code']).last()
        BalanceSheet = BalanceSheet.drop(['report_period'],axis=1)
