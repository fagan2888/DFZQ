...
class InsurIndicators(FactorBase):
    def __init__(self):
        super().__init__()
    ...
    def JOB_factors(df, field, codes, calendar, start, save_db):
        ...
        for code in codes:
            ...
            if r == 'No error occurred...':
                ...
                save_res.append('Insurance Field: %s  Error: %s' % (field, r))
        return save_res
    ...
    def cal_factors(self, start, end, n_jobs):
        ...
        query = "select ANN_DT, S_INFO_WINDCODE, REPORT_PERIOD, TOTAL_INVESTMENT_YIELD, STATEMENT_TYPE " \
                "from wind_filesync.AShareInsuranceIndicator " \
        ...
        insur = pd.DataFrame(self.rdf.curs.fetchall(),
                             columns=['date', 'code', 'report_period', 'total_investment_yield', 'type'])
        # 同一code，同一date，同一report_period，同时出现type1，2，3时，取type大的
        insur['type'] = insur['type'].apply(lambda x: '2' if x == '408001000' else ('3' if x == '408005000' else '4'))
        insur = insur.sort_values(by=['code', 'date', 'report_period', 'type'])
        insur['date'] = pd.to_datetime(insur['date'])
        insur['report_period'] = pd.to_datetime(insur['report_period'])
        insur['total_investment_yield'] = \
            insur.groupby(['code', 'report_period'])['total_investment_yield'].fillna(method='ffill')
        # ***************************************************************************
        # 需要的field
        fields = ['total_investment_yield']
        # 处理数据
        ...
        for f in fields:
            ...
            df = pd.DataFrame(insur.dropna(subset=[f]).groupby(['code', 'date', 'report_period'])[f].last()) \
                .reset_index()
            ...
            with parallel_backend('multiprocessing', n_jobs=n_jobs):
                res = Parallel()(delayed(InsurIndicators.JOB_factors)
...
if __name__ == '__main__':
    npl = InsurIndicators()
    r = npl.cal_factors(20090101, 20200629, N_JOBS)