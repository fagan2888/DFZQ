...
class div_pred_statistic:
                    ...
                    for i in range(len(div_this_year_df)):
                        ...
                        if div_dt < self.last_trading_date:
                            if mode == 'rdf':
                                ...
                                stk_weight_series = idx_constituent_that_day.loc[idx_constituent_that_day['constituent_code'] == code, 'weight']
                            ...
                            bonus_point = self.cal_div_point(div_pretax,stk_close_that_day, idx_close_that_day, stk_weight_that_day)
                        else:
                            bonus_point = self.cal_div_point(div_pretax,stock_close,index_close,stock_weight)
                        temp_df = pd.DataFrame({'股票代码': [stock_code_processing], '股票名称': [stock_name_processing],
                                                '状态': [status], '是否预测': [if_predicted], '每股转送': [conversed_ratio],
    ...
    def constituent_div_prediction(self, dt_input,index_type,years_back=2,mode='rdf'):
        ...
        index_close_df = self.rdf.get_index_close(index_type,self.last_trading_date-relativedelta(years=1),self.last_trading_date)
        ...
        for code in index_constituent_list:
            ...
            prediction_df_list.append(self.process_prediction(code,stock_div_df,stock_weight,index_close_df,index_type,mode))
    ...
    def modify(self, raw_df, index_type):
        ...
        for idx, row in revising_df.iterrows():
            code = row['股票代码']
            pre_date = row['预案公告日']
            if not raw_df.loc[(raw_df['股票代码']==code)&(raw_df['预案公告日']==pre_date), '是否预测'].iloc[0]:
                pass
            else:
                if code in modified_code_list:
                    tmp_df = raw_df.loc[(raw_df['股票代码']==code)&(raw_df['预案公告日']==pre_date),:].copy()
                    ...
                    if row['税前派息'] == row['税前派息']:
                        multi = float(row['税前派息'].split('*')[1])
                        modified_df.loc[(modified_df['股票代码']==code)&(modified_df['预案公告日']==pre_date),'税前派息'] = \
                            multi * modified_df.loc[(modified_df['股票代码']==code)&(modified_df['预案公告日']==pre_date),'税前派息']
                    if row['分红点数'] == row['分红点数']:
                        multi = float(row['分红点数'].split('*')[1])
                        modified_df.loc[(modified_df['股票代码']==code)&(modified_df['预案公告日']==pre_date),'分红点数'] = \
                            multi * modified_df.loc[(modified_df['股票代码']==code)&(modified_df['预案公告日']==pre_date), '分红点数']
                    if row['每股转送'] == row['每股转送']:
                        modified_df.loc[(modified_df['股票代码']==code)&(modified_df['预案公告日']==pre_date),'每股转送'] = row['每股转送']
                    if row['股权登记日'] == row['股权登记日']:
                        modified_df.loc[(modified_df['股票代码']==code)&(modified_df['预案公告日']==pre_date),'股权登记日'] = row['股权登记日']
                    if row['派息日'] == row['派息日']:
                        modified_df.loc[(modified_df['股票代码']==code)&(modified_df['预案公告日']==pre_date),'派息日'] = row['派息日']
